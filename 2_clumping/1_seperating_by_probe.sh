...
#get the probe names
cut -d',' -f1 /mnt/iusers01/bk01/m20349jk/causal_hypertension/eQTL/pval_01/data/map_to_rsId/sorted_eqtl_results_p0.01.csv | sort | uniq > individual_probes.txt
#prints list of the unique probes in the file
#there are 17540
split -l 1754 individual_probes.txt individual_probes
...
#Run on server
#make one for all individual_probesaa to individual_probesae (call them sep_by_probea.txt to sep_by_probee.txt)
#!/bin/bash --login
#$ -cwd 
for OUTPUT in $(cat individual_probesaj)
do
  mkdir ${OUTPUT}
  cd ${OUTPUT}
  ...
  sed -i "1s/^/$(head -n1 /mnt/iusers01/bk01/m20349jk/scratch/single_probes/combined_individual_eQTL.txt)\n/" individual_SNPs.csv #add heading to the SNP table
  echo "${OUTPUT} completed"
  cd ..
done


qsub sep_by_probea.txt
qsub sep_by_probeb.txt
qsub sep_by_probec.txt
qsub sep_by_probed.txt
qsub sep_by_probee.txt
qsub sep_by_probef.txt
qsub sep_by_probeg.txt
qsub sep_by_probeh.txt
qsub sep_by_probei.txt
qsub sep_by_probej.txt
